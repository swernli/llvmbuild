...
. (Join-Path $PSScriptRoot CommonBuild.ps1)
...
class CMakeConfig
{
    ...
    CMakeConfig([string]$plat, [string]$config, [string]$baseBuild, [string]$srcRoot, [bool]$onWindows)
    {
        ...
        {
            $VsInstance = Find-VSInstance -Prerelease
            if (!$VsInstance)
            {
        ...
        # $this.GenerateCommandArgs += "-DCMAKE_BUILD_TYPE=$($this.ConfigurationType)"
...
function global:Invoke-CMakeGenerate( [CMakeConfig]$config )
{
    ...
    New-Item -ItemType Directory $config.BuildRoot | Out-Null
