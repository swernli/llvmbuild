...
#>
    ...
    {
        Write-Information "Can't find on path, fallback to using: '$exeName'"
        $path = $exeName
    }
    return $path
}
...
function Find-VSInstance([switch]$PreRelease, $Version = '[15.0, 17.0)')
{
<#
.SYNOPSIS
...
#>
    if ($IsWindows)
    {
        ...
        $existingModule = Get-Module -ErrorAction SilentlyContinue VSSetup
        if(!$existingModule)
        {
        ...
        $vs = Get-VsSetupInstance -Prerelease:$PreRelease |
                Select-VsSetupInstance -Version $Version |
                select -Last 1
